use super::{
    decls::{AliasDef, ConstDef, EnumDef, FuncDef, ImplBlock, StructDef, TraitDecl, UnionDef},
    utils::{Braces, CommaSep, Seq, check_enum},
};
use crate::{
    lexer::TokenKind,
...
impl ParseNode for ModuleItem {
    // WithDoc<WithVis<AliasDef>>
    // WithDoc<WithVis<ConstDef>>
    // WithDoc<WithVis<EnumDef>>
    // WithDoc<WithAbi<FfiBlock>>
    // WithDoc<WithVis<WithAbi<FuncDef>>>
    // WithDoc<ImplBlock>,
    // WithVis<ImportStmt>
    // WithDoc<WithVis<StructDef>>
    // WithDoc<WithVis<TraitDef>>
    // WithDoc<WithVis<UnionDef>>
    //
    // WithDoc
    //   WithVis
    //     WithAbi
    ...
    fn parse(context: &mut ParseContext) -> usize {
        match Self::check(context.peek()) {
            ...
            CheckResult::Always(7) => {
                context.parse::<TraitDecl>();

                7
            }
            CheckResult::Always(8) => {
