...
12. add crash/miscompile bundle export as part of the reducer and wrong-code workflow: every crash, verifier failure, report contradiction, or miscompile should be capturable as a stable bundle containing minimized source, expected/actual behavior, flags, relevant dumps, and artifact/report outputs — the bundle contract is now live (`docs/BUG_BUNDLE.md`, `scripts/tests/capture_wrong_code_bundle.sh`, `make test-bundle-smoke`), wraps the existing `concrete debug-bundle` for pipeline-state capture and layers on corpus-specific files (command.txt, predicate.txt, compiler-version.txt, stdout/stderr capture, opportunistic reports), and integrates with the reducer via `minimize_wrong_code.sh ... --bundle <dir>` so the full pipeline is `new bug → reduce → bundle → manifest entry → permanent regression`
...
14. make reducer/minimizer workflow part of normal compiler hardening earlier, not only late backend work: minimize SSA verifier failures, wrong-code cases, fact/report mismatches, and crashers into named regressions; the workflow is now live (`docs/REDUCER_WORKFLOW.md`, `scripts/reduce/expect-*.sh`, `scripts/tests/minimize_wrong_code.sh`, `make test-reducer-smoke`), driven by `Concrete.Reduce` extended with an `external:<cmd>` predicate so all four user-facing predicates (error-code, runtime-output, oracle-mismatch, report-contains) funnel through the same syntax-aware shrinker. Demoed on WC-0015 (27 → 20 lines). Remaining work is to add manifest entries with a `Reduction` block per the workflow doc as new bugs are minimized
...
16. build and maintain a named wrong-code regression corpus: every discovered miscompile, codegen bug, obligation bug, checker soundness bug, and proof-pipeline regression should land as a stable reproduction, not just disappear into the general suite — the corpus is live (`docs/WRONG_CODE_CORPUS.md`, `tests/wrong-code/manifest.toml`, `make test-wrong-code`), the contract has been proven on both fixed cases and one open-then-fixed case (WC-0004), all `bug_*.con` and curated high-risk adversarial cases are bulk-registered (WC-0001 through WC-0022, 22 PASS / 0 FAIL); remaining work is to treat every newly discovered bug as a manifest entry on the day it's found and to add a corpus-update reflex to triage workflow
...
11. add pass-by-pass verifier gates across the named compiler boundaries instead of trusting downstream failures to reveal upstream corruption: parsed/resolved/import summaries, checked/elaborated Core, lowered IR, and SSA should each have structural/self-consistency verifiers that run in normal development and CI so the compiler catches "it lied to itself" bugs as early as possible — the four downstream boundaries (post-elab, post-mono, post-lower, post-cleanup) are now wired through `Pipeline.runVerifyGates`, surfaced via `concrete <file> --report verify`, and gated across the curated corpora via `make test-verify-gates` (`docs/VERIFY_GATES.md`); first landing reports 78 PASS / 0 FAIL with 2 documented `try_` placeholder warnings. Remaining work is to add structural verifiers at the earlier boundaries (parsed/resolved/import-summary/post-check) when concrete bugs motivate them, per the never-delete rule
