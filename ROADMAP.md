...
16. build and maintain a named wrong-code regression corpus: every discovered miscompile, codegen bug, obligation bug, checker soundness bug, and proof-pipeline regression should land as a stable reproduction, not just disappear into the general suite — the corpus contract is now live (`docs/WRONG_CODE_CORPUS.md`, `tests/wrong-code/manifest.toml`, `make test-wrong-code`) and seeded; remaining work is to register the rest of the existing `bug_*.con` and `adversarial/<area>/<name>.con` cases, add per-case notes for each, and treat every newly discovered bug as a manifest entry on the day it's found
