...
setup-hooks: ## Install the versioned git hooks (one-time, per clone) — pre-push then runs the CI gate set automatically
	git config core.hooksPath .githooks
	@echo "hooks installed: .githooks (pre-push runs the CI gate set)"

pre-push: ## Run exactly what the pre-push hook runs (build + every CI gate, parallel). Usable without installing hooks.
