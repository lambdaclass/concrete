...
2. **Add saturating intrinsics**: `saturating_add`, `saturating_sub`, `saturating_mul` emitting LLVM `llvm.*.sat` intrinsics. **[add/sub DONE — 2026-06-24; ROADMAP #10 Stage 2.2: `BinOp.saturatingAdd/Sub` → `llvm.{s,u}{add,sub}.sat.iW` (signed/width-mangled, statically declared); integer-only, same-type; interpreter clamps to range; gate `scripts/tests/check_saturating_arith.sh`.]** `saturating_mul` has no direct `*.sat` intrinsic — it lands in Stage 2.2b on the shared `*.with.overflow` + clamp/select infrastructure (the same infra the checked flip uses).
