...
|---:|---|---|---|
| G-01 | Body fingerprint check | `hash` of the source `CExpr/CStmt` fingerprint matches the in-source `#[proof_fingerprint]`.  Drift downgrades obligation status to `stale`. | `fingerprint_determinism`: source produces a single canonical fingerprint string; equivalent source produces same fingerprint. |
