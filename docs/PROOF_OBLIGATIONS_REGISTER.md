...
| R-08 | `call fn args` | function call (FnTable lookup) | parse_validate | `call_preservation` — **extraction discharged against `cExprToPExpr` 2026-05-30** (after mapM lift) |
| R-09 | `structLit name fields` | `Point { x: 1, y: 2 }` | parse_validate (parse_header Ok) | `struct_lit_preservation` — **extraction discharged against `cExprToPExpr` 2026-05-30** (after mapM lift) |
...
| R-11 | `enumLit ename var fields` | `Result::Ok { value: v }` | parse_validate (parse_header Err) | `enum_lit_preservation` — **extraction discharged against `cExprToPExpr` 2026-05-30** (after mapM lift) |
| R-12 | `match_ scrutinee arms` (`PMatchPat`: `enumPat` / `litPat` / `varPat`) | `match c { Pat => body, ... }` | parse_validate (`error_code`) | `match_preservation` — **extraction discharged against `cExprToPExpr` 2026-05-30** (after mapM lift); per-arm preservation is a separate follow-up |
...
| R-15 | `arrayLit elems` | `[0, 0, 0, ...]` | fixed_capacity (`ring_new`) | `array_lit_preservation` — **extraction discharged against `cExprToPExpr` 2026-05-30** (after mapM lift) |
...
| R-18 | `while_ cond assigns cont` (flat-assign body) | `while cond { x = ...; y = ...; }` | parse_validate (`compute_checksum`), fixed_capacity (`compute_tag`) | `while_flat_preservation` — **extraction discharged against `cStmtsToPExprK` 2026-05-30**; termination via fuel only — `bounded` profile enforces real termination at Check |
