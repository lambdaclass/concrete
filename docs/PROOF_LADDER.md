...
| `hmac_sha256` (key-prep + ipad/opad + three `sha256_hash` calls) refines `Sha256Spec.hmac` | **next** — the final *composition* proof; reuses `sha256_hash_refines_spec` and the existing loop/store infrastructure, no new machinery (ROADMAP Phase 8 / bar #2) |
