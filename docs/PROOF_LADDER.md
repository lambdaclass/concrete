...
| Full `hmac_sha256(k, k_len, m, m_len)` refines `Sha256Spec.hmac` end-to-end (the `if k_len>64` key-prep branch + ipad/opad xor + message embed + three `sha256_hash` calls + digest copies) for every `k_len ≤ 128`, `m_len ≤ 256` | **kernel-checked over a faithful hand model** (`hmac_sha256_refines_spec`; `hmac_linear` for the post-key-prep body, `kp_else`/`kp_if` for the branch). **NOT yet bar #2 / graduation-grade**: the proof is about a hand-transcribed `PExpr` that does *not* equal the actual extracted source body, so it is not tied through the registry spec-drift path the way the other flagships' theorems are. The extraction differs structurally — `gt k_len 64` (not `lt 64 k_len`), the key-prep `if` is a statement that mutates `kp` in place and **duplicates the whole linear tail into both branches** (no SSA rebind), and `add i 64` (not `add 64 i`). Graduation (HMAC bar #2 at full standard) is **blocked** on re-proving the refinement about the exactly-extracted body and registering it through spec-drift so source drift breaks the claim. |
