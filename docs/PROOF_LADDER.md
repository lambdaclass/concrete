...
| `hmac_sha256` **top-level assembly** (the `if k_len>64` key-prep branch + xor loop + message copy + three `sha256_hash` calls + digest copies) refines `Sha256Spec.hmac` | **next** — the final wiring; composes the shipped infrastructure above, no new techniques (ROADMAP Phase 8 / bar #2) |
