...
| Full `hmac_sha256(k, k_len, m, m_len)` refines `Sha256Spec.hmac` end-to-end (the `if k_len>64` key-prep branch + ipad/opad xor + message embed + three `sha256_hash` calls + digest copies) for every `k_len ≤ 128`, `m_len ≤ 256` | **shipped** (`hmac_sha256_refines_spec`; `hmac_linear` for the post-key-prep body, `kp_else`/`kp_if` for the branch) — **HMAC bar #2 closed** |
